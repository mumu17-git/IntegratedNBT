...
public abstract class NBTTreeViewer {
    ...
    private static final int HIGHLIGHT_COLOR = 0xCCCCCC; // Dark gray
    private static final int SELECTED_COLOR = 0xA0A0A0; // Lighter Dark gray
    ...
    private static final int COMPLEX_COLOR = 0xDCAA11; // Gray
    ...
    private boolean renderKVPair(
        ...
        if (value.isEmpty()) {
            matrixStack.drawString(fontRenderer, label, this.currentX, this.currentY, LABEL_COLOR);
        } else {
            int valueX = matrixStack.drawString(
                fontRenderer,
                label + ": ",
                this.currentX,
                this.currentY,
            ...
            matrixStack.drawString(fontRenderer, value, valueX, this.currentY, valueColor);
        }
        return isHovering;
    }

    private void renderExpandableButton(GuiGraphics matrixStack, boolean expanded) {
        boolean hovering = (
            this.mouseX >= this.currentX &&
        ...
        part.renderTo(matrixStack, matrixStack.pose(), this.currentX, this.currentY, EXPAND_COLOR);
        this.currentX += EXPAND_BUTTON_SIZE + EXPAND_BUTTON_RIGHT_MARGIN;
    }

    public void render(GuiGraphics matrixStack, Tag nbt, int absMouseX, int absMouseY) {
        ...
        try {
            matrixStack.pose().translate(this.left, this.top - this.renderScroll, 0);
            if (nbt == null) {
            ...
            matrixStack.pose().translate(0, this.renderScroll, 0);
    ...
    private void renderEmpty(GuiGraphics matrixStack) {
    ...
    private void renderNode(GuiGraphics matrixStack, String label, Tag node) {
